...
<title>Untuk Bang Efung & Kak Zia 💍</title>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;1,400;1,500&family=Lato:wght@300;400;700&family=Dancing+Script:wght@500;600&family=Caveat:wght@400;600;700&display=swap" rel="stylesheet" />
<style>
...
/* Bubble user — kanan, warna emas hangat */
  ...
  <div class="envelope-wrapper" id="envelopeWrapper">
    <div class="envelope-body">
      ...
      <div class="envelope-address">
        ...
        <span class="envelope-name">Bang Efung & Kak Zia</span>
      </div>
    </div>
  </div>
...
function addChatMessage(role, text, quotedText = null, quotedSender = null, usedModel = null) {
  ...
  const label = role === 'bot' ? '✦ Birru (AI dari Haeru)' : 'Kamu';
...
function addVideoMessage() {
  ...
  el.innerHTML = `
    <div class="msg-label">✦ Birru (AI dari Haeru)</div>
...
function addWaButton(userName) {
  ...
  const messages = {
    ...
    "amriah fauziah": `Halo Haeru! Ini Kak Zia. Baru buka kado dari kamu — terima kasih ya, sangat berkesan 🤍`,
...
function detectMusicIntent(text) {
  ...
  const listKw  = ['pilih sendiri', 'ada pilihan', 'daftar lagu', 'lagu apa aja', 'lagu apa saja', 'ada lagu lain', 'lagu lainnya', 'mau yang lain', 'ada lagi', 'list lagu', 'lagu lain'];
  const playKw  = ['mainkan musik', 'putar musik', 'minta musik', 'play musik', 'musik dong', 'mainkan lagu', 'putar lagu', 'minta lagu', 'play lagu', 'lagu dong', 'mainkan piano', 'putar piano', 'piano dong', 'mainkan piano', 'dengerin musik', 'dengerin lagu', 'pengen denger musik', 'pengen denger lagu'];
  const stopKw  = ['stop musik', 'hentikan musik', 'pause musik', 'stop lagu', 'hentikan lagu', 'pause lagu'];
  ...
  // Deteksi pilih nomor lagu (hanya angka)
  if (/^\d+$/.test(text.trim()))          return 'select';
  return null;
}
...
async function handleMusicIntent(intent, text) {
  ...
  if (intent === 'select') {
    ...
    if (!playlist.length) {
      ...
      addAudioPlayerBubble(playlist[idx]);
    } else {
      addChatMessage('bot', 'Nomornya di luar daftar, Kak — ketik sesuai yang aku tampilkan ya. 😄');
    }
    return;
  }
...
async function runHaeuCameo() {
  ...
  // 4. Tunggu sinyal Kak Zia sudah baca (max 20 detik)
  ...
  birruTyping.classList.remove('visible');
  await new Promise(r => setTimeout(r, 180));
  const pianoMsg = "Senang akhirnya bisa ngobrol, Kak. Kalau mau cerita, tanya soal keluarga, atau sekadar ngobrol santai — aku di sini. Oh iya, aku juga bisa mainkan piano kalau mau, boleh banget didengerin kapanpun. 🎵";
